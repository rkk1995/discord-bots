...
def transform_url(url):
    ...
    if any(domain in url for domain in ('fxtwitter.com', 'kkinstagram.com', 'ddinstagram.com')):
    ...
    if ('x.com' in url or 'twitter.com' in url) and '/status' in url:
         url = re.sub(r'https://(www\.)?(x\.com|twitter\.com)', 'https://fxtwitter.com', url)
...
def handle_links(message) -> tuple[bool, str]:
    ...
    url_pattern = re.compile(r'<?(http[s]?://(?:www\.)?(?:x\.com|twitter\.com|instagram\.com|vxtwitter\.com|fxtwitter\.com|ddinstagram\.com|kkinstagram\.com)[^\s>]+)>?')
